        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-10">
          ...
          <div className="space-y-2">
            <button 
              ...
              className={`w-full flex items-center gap-3 px-6 py-4 rounded-2xl transition-all font-bold text-sm text-left ${activeTab === 'perfil' ? 'bg-white border border-slate-200 shadow-sm text-emerald-600' : 'text-slate-500 hover:bg-white/50'}`}
            >
              <User size={18} />
              Meu Perfil
            </button>
            <button 
              ...
              className={`w-full flex items-center gap-3 px-6 py-4 rounded-2xl transition-all font-bold text-sm text-left ${activeTab === 'notificacoes' ? 'bg-white border border-slate-200 shadow-sm text-emerald-600' : 'text-slate-500 hover:bg-white/50'}`}
            >
              <Bell size={18} />
              Notificações
            </button>
            <button 
              ...
              className={`w-full flex items-center gap-3 px-6 py-4 rounded-2xl transition-all font-bold text-sm text-left ${activeTab === 'seguranca' ? 'bg-white border border-slate-200 shadow-sm text-emerald-600' : 'text-slate-500 hover:bg-white/50'}`}
            >
              <Lock size={18} />
              Segurança
            </button>
            ...
            {activeTab === 'perfil' ? (
              <form onSubmit={handleSubmit} className="bg-white p-8 md:p-12 rounded-[3.5rem] border border-slate-100 shadow-sm space-y-8">
                ...
                <div className="flex flex-col md:flex-row items-center gap-8 mb-10 pb-10 border-b border-slate-50">
                  <div className="relative group cursor-pointer">
                    <label className="cursor-pointer">
                      <div className="w-32 h-32 rounded-[2.5rem] bg-slate-100 ring-8 ring-slate-50 flex items-center justify-center text-slate-300 transition-all group-hover:ring-emerald-50 overflow-hidden">
                        {formData.avatar_url ? (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-2">
                    <label className="text-xs font-black text-slate-400 uppercase tracking-widest ml-1">WhatsApp Padrão</label>
                    <div className="relative group">
              ...
              <div className="bg-white p-12 rounded-[3.5rem] border border-slate-100 shadow-sm flex flex-col items-center justify-center text-center py-20">
                ...
                <h3 className="text-2xl font-black text-slate-800 mb-2">Em Construção</h3>
                <p className="text-slate-400 font-medium max-w-sm">Esta funcionalidade estará disponível em breve no próximo update do VetHome.</p>
              </div>
            )}
          </div>
        </div>
